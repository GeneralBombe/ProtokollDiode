...
\begin{center}
	Hinweis:  Der Sperrstrom bei 0,5V ist beim Messen zwischen \SI{0.25}{\nano\ampere} und \SI{0.4}{\nano\ampere} geschwankt.
\end{center}
...
\end{tabular}
\caption{Vorwärtestrom \(I/\si{\ampere}\) in Abhängigkeit von der Spannung \(U_R/\si{\volt}\).}
\label{tab:uv-cf}
\end{table}
...
\begin{figure}[H] % [H] forces the figure to be placed exactly where it appears in the text
	...
	\caption{Nichtlineare Regression in Python. Rote Linie $f(U)= \frac{1}{C_S^\frac{1}{M}}$ bei Stufenfaktor $M=0.33$.}
\end{figure}

\[
C_S = \frac{C_{S0}}{(1+\frac{U}{U_D})^M}\quad f(U) = \frac{1}{C_S^\frac{1}{M}}
\]
Der Stufenfaktor wurde durch lineare Regression der Kapazitäts-Spannungs-Daten, abhängig von M linearisiert. Die beste Linearisierung entspricht in etwa dem Stufenfaktor. 
...
Stufenfaktor von $M \approx 0,33$ ist ein Indiz für einen abrupten pn-Übergang. \\
Das hierfür verwendete Python Skript findet sich im Anhang.
...
Für Vergleich in Übersichtsdarstellung siehe Abbildung B.2.2.
\[
\left.
\begin{array}{l}
W_g = h \cdot f \\
...
Je kleiner $I_s$ bzw. je größer $W_g$, umso größer muss $U$ sein, um einen Strom $I$ zu halten.\\  
...
Da im Datenblatt nur sehr unpräzise Aussagen über die Forward-Spannung getroffen werden und keine Aussage über den Bahnwiderstand gemacht wird, ist es schwierig, die Werte zu vergleichen.
...
Für zeichnerisch dargestellte Idealkennlinie siehe B.2.1.
...
In der gemessenen Kennlinie werden reale Parameter wie Leitungswiderstand, nichtideales Verhalten des Halbleiters sowie Temperaturabhängigkeiten abgebildet.
Der Emissionskoeffizient passt dabei die Idealkennlinie etwas an die gemessene an. Trotzdem besteht noch eine gute Übereinstimmung mit der idealen Kennlinie.
