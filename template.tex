...
\]
Zur Bestimmung des Dotierstoffprofils eines pn-Übergangs wurde die Sperrschichtkapazität $C_S$ in Abhängigkeit von der angelegten Sperrspannung $U_R$ ausgewertet. Die Messdaten wurden in Abbildung~\ref{fig:Sperrschichtkap} aufgetragen.
Durch nichtlineare Regression mit variablen $M$ werden die Kapazitäts Spannungs Daten linearisiert. Durch diese näherung wird der Schnittpunkt der Ausgleichsgeraden mit der Spannungsachte berechnet. Dieser entspricht $U_D$ (hier $U_D \approx \SI{0.275}{\volt}$). 
Das $M$, für welches die nichtlineare Regression die beste Näherung liefert, entspricht dem Stufenfaktor. \\
Das Python-Skript kann im Anhang gefunden werden.

Der Stufenfaktor von $M=0.33$ ist ein Indiz für einen linearen pn-Übergang.
